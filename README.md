...
This repository provides a **complete and faithful reproduction** of the [paper](https://arxiv.org/abs/2504.16427) and the open source [project](https://github.com/thuiar/MMLA/tree/main?tab=readme-ov-file):

> **MMLA: A Unified Benchmark for Multimodal Language Analysis**  
> THUIAR, ACL 2024
...
The 72B model is only applicable to multi-GPU. The single-GPU can only run models of **7B and below**, and must adjust parameter in yaml file to prevent throwing out an error.
## 📁 Project Structure
```python
MMLA/  
├── data/
...
## 3. IT
### 3.1 Data preparation
In IT process, all 9 train files and dev files should be combined together, and like SFT process, the combined files `IT_all9_train_llamafactory.json` and `IT_all9_dev_llamafactory.json`need to be converted into LLaMA Factory format
```python
...
```
### 3.3 IT Inference
Merge LoRA and get a IT inference model
```python
