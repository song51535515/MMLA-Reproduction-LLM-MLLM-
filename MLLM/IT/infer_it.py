...
def evaluate_videos(args, model, processor, json_file_path, video_path, labels_dict, logger):
    ...
    for item in tqdm(data, desc=f"Predicting {args.task}"):
        ...
        # ---- FIX: ensure chat_template is available ----
        if getattr(processor, "chat_template", None) is None:
            # processors in some versions don't inherit tokenizer.chat_template automatically
            if hasattr(processor, "tokenizer") and getattr(processor.tokenizer, "chat_template", None):
